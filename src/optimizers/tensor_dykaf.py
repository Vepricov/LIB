...
def hopm(g, n_iters=100):
    assert len(g.shape)==4
    rules = ['ijkl,j,k,l->i',
             'ijkl,i,k,l->j',
             'ijkl,i,j,l->k',
             'ijkl,i,j,k->l']
    v = [torch.randn(g.shape[i], dtype=g.dtype, device=g.device) for i in range(len(g.shape))]
    for _ in range(n_iters):
        for k in range(4):
            v[k] = torch.einsum(rules[k], g, *[v[t] for t in range(4) if t!=k])
            v[k] /= torch.norm(v[k])
    s = torch.sum(g * torch.einsum("i,j,k,l->ijkl", *v))
    for i in range(4):
        v[i] *= s**0.25
    return [torch.outer(vec, vec) for vec in v]
...
def init_precond(g, Ls, init="kron", max_precond_dim=10000):
    d = g.ndim
    if init == "eps":
        L, R = [], []
        eps = 1e-3
...
# def tensor_unfolding(g, k):
#     d = g.ndim
#     k_shape = g.shape[k]
#     order = [k] + list(range(0, k)) + list(range(k+1, d))
#     return torch.permute(g, order).reshape(k_shape, -1).contiguous()
...
#     tensors = [g] + [Ls[i] for i in range(d) if i != k]
#     return torch.einsum(eq, *tensors)


def tensor_proj_split(g: torch.Tensor, L: List[torch.Tensor], beta):
    # g: n-dimensional tensor
    # L: list of matrices of norm 1
    assert len(g.shape)==4
    ...
    rules = ['ijkl,sabc,ja,kb,lc->is',
             'ijkl,asbc,ia,kb,lc->js',
             'ijkl,absc,ia,jb,lc->ks',
             'ijkl,abcs,ia,jb,kc->ls']
    for i in range(4):
        C = torch.einsum(rules[i], g, g, *[L[t] for t in range(4) if t!=i])
        L[i] = S * L[i] + C
        S_hat = torch.norm(L[i])
        L[i] /= S_hat
        S = S_hat - torch.norm(L[i] * C)
...
class TensorDyKAF(torch.optim.Optimizer):
    """
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(grad.shape) == 2 and self.adam_rank_one:
                    ...
                    ##### DEFAULT ADAM UPDATE #####
                    ###############################
                    # print(torch.linalg.norm(grad_projected))
                ...
                if (
                    ...
                    # print(torch.linalg.norm(H_approx), torch.linalg.norm(hess), beta2)
        ...
        for m in matrix:
            ...
            try:
                ...
                if 'cusolver' in str(e).lower():
                    m_cpu = m.cpu()
                    _, Q = torch.linalg.eigh(
                        m_cpu + 1e-30 * torch.eye(m_cpu.shape[0])
                    )
                    Q = Q.to(m.device)
                else:
                    # Try float64 as before
                    _, Q = torch.linalg.eigh(
